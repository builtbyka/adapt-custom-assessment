define(function(require) {
    ...
    QuestionBank.prototype = {
        ...
        customOrder: function(qo, qb) {
            var order = [],
            newOrder = [];
            for(var i = 1; i <= this._numQuestionBlocks; i++){
                order[i] = [];
            }
            for(var j = 0; j < qb.length; j++){
                order[qb[j].get('_assessment')._quizBankID].push(qb[j]);
            }
            for(var k = 0; k < qo.length; k++){
                newOrder.push(order[qo[k]][0]);
                order[qo[k]].shift();
                //console.log(qo[k]);
            }
            return newOrder;
